...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * table初始化容量默认值，值必须是2的次方数
     * (例如, 最小值是1) 最大值是 MAXIMUM_CAPACITY.
     */
    ...
    static class Node<K, V> implements Map.Entry<K, V> {
        ...
        public final boolean equals(Object o) {
            ...
            // 校验传入key是否为null
            toBeComparedEntry = (Map.Entry<?, ?>) o;
            key = toBeComparedEntry.getKey();
            if (key == null) {
                return false;
            }

            // 校验传入value是否为null
            value = toBeComparedEntry.getValue();
            if (value == null) {
                return false;
            }
            ...
            // 判断value是否相等
            if (value != this.val && !value.equals(this.val)) {
                return false;
            }
